...
const dbConnString = "root:mysql-local@/test"
...
func dispatchWorkers(db *sql.DB, jobs <-chan []interface{}, wg *sync.WaitGroup) {
	...
	for workerIndex := 0; workerIndex <= totalWorker; workerIndex++ {
		...
		go func(workkerIndex int, db *sql.DB, jobs <-chan []interface{}, wg *sync.WaitGroup) {

			counter := 0

			for job := range jobs {
				doTheJob(workkerIndex, counter, db, job)
				wg.Done()
				counter++
			}
...
// read file csv then send to workers
func readCsvFilePerLine(csvReader *csv.Reader, jobs chan<- []interface{}, wg *sync.WaitGroup) {

	for {

		// use Read() rather than ReadAll()
		// ReadAll() for read all and parse at once, when we have huge CSV file, may not fir in memory
		...
		// first data (which mean header)
		if len(dataHeaders) == 0 {
			dataHeaders = row
			continue
		}

		// second data and next processed
...
func main() {
	...
	readCsvFilePerLine(csvReader, jobs, wg)
